...
<div id='ShowBookingHotels'>
	<div class="hms-header-section">
		...
		<div class="HMS-nav-bar">
			...
			<a class="hms-nav-links active" href="BookAHotel.php">Book Hostel</a>
			<a class="hms-nav-links" href="Greivance.php">Service And Maintainance</a>
		</div>
	</div>
	<div class="sortOptions">
		<h3>Sort By: </h3>
		<button class="btn btn-primary" onclick="ShowListOfHostels()">Beds Vacant</button>
		<button class="btn btn-primary" onclick="ShowListOfHostelsByPriceAsc()">Price: Low to High</button>
		<button class="btn btn-primary" onclick="ShowListOfHostelsByPriceDesc()">Price: High to Low</button>
		<button class="btn btn-primary" onclick="ShowListOfHostelsByAC()">AC Available</button>
		<button class="btn btn-primary" onclick="ShowListOfHostelsStdPerBed()">Students Per Bed</button>
	</div>
...
<script>
	...
	function ShowListOfHostels(){
		...
		<?php 
			...
			$query="SELECT * FROM Hotels_Info order by Beds_Vacant";
	...
	function ShowListOfHostelsByPriceAsc(){
		$(".hostelsOuterDiv").remove();
		<?php 
			$user_name = "root";
			$password = "";
			...
			$conn=mysqli_connect('localhost','root','','HMSystem') or die("could not connect to internet");
			$selected=mysqli_select_db($conn,"HMSystem");
			$query="SELECT * FROM Hotels_Info order by Cost_Per_Year";
			$result=mysqli_query($conn,$query);
	...
	function ShowListOfHostelsByPriceDesc(){
		$(".hostelsOuterDiv").remove();
		<?php 
			$user_name = "root";
			$password = "";
			...
			$conn=mysqli_connect('localhost','root','','HMSystem') or die("could not connect to internet");
			$selected=mysqli_select_db($conn,"HMSystem");
			$query="SELECT * FROM Hotels_Info order by Cost_Per_Year DESC";
			$result=mysqli_query($conn,$query);
	...
	function ShowListOfHostelsByAC(){
		$(".hostelsOuterDiv").remove();
		<?php 
			$user_name = "root";
			$password = "";
			...
			$conn=mysqli_connect('localhost','root','','HMSystem') or die("could not connect to internet");
			$selected=mysqli_select_db($conn,"HMSystem");
			$query="SELECT * FROM Hotels_Info order by Ac_NonAc";
			$result=mysqli_query($conn,$query);
	...
	function ShowListOfHostelsStdPerBed(){
		$(".hostelsOuterDiv").remove();
		<?php 
			$user_name = "root";
			$password = "";
			...
			$conn=mysqli_connect('localhost','root','','HMSystem') or die("could not connect to internet");
			$selected=mysqli_select_db($conn,"HMSystem");
			$query="SELECT * FROM Hotels_Info order by Students_per_room";
			$result=mysqli_query($conn,$query);
